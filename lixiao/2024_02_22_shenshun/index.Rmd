...
```{r include = F, eval = F}
deparse_mail()
info <- items(belong = odate(3), eval = ic(1, 3, 2, 0, 0))
...
lm.pps <- job_limma_normed(pb.pps@params$counts, metadata)
lm.pps <- step1(lm.pps)
lm.pps <- step2(lm.pps, Cancer - Control, use = "P")
lm.pps@plots$step2$p.valcano$`Cancer - Control`
tops <- lm.pps@tables$step2$tops$`Cancer - Control`

pb.pps@object
tops <- map(tops, "rownames", pb.pps@object$data, "UniProt_Knowledgebase_ID", "Gene_name")
```
...
这里尝试根据 KEGG top 10 通路的富集基因缩减 PPI
