...
```{r include = F, eval = F}
deparse_mail()
info <- items(belong = c(odate(3), odate(4)), eval = list(ic(2, 3, 3, 0, 0), ic(2, 0, 0, 3, 0)))
show.ic(info)
...
cp <- step3(cp)
cp@params$groups
cp@params$groups_status
cp@params$layouts
cp <- upload(cp)

cp <- login(cp)
cp <- pull(cp, "test")
cp@params$tmp_models

x <- get_table.html(cp$link$getPageSource()[[1]])
```
