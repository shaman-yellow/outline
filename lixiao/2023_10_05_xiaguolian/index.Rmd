...
autor(gse@plots$step3$p.pie_insDb)
```

```{r eval = T, echo = F, results = "asis"}
#| intersected-genes-of-DEGs-with-Ferrdb
autor(gse@tables$step3$table_insDb, key = "symbol")
```
...
autor(gse@plots$step3$p.code)
```

```{r eval = T, echo = F, results = "asis"}
#| table-of-All-DEGs-of-GSEA-enrichment-via-FerrDb-V2
autor(gse@tables$step3$table_gsea)
```
...
gse@step <- 2L
gse <- step3(gse, db_fe, 1, map = "driver|supp|marker")
gse@plots$step3$p.code
```
...
```{r}
en5 <- job_enrich(list(ids = gse@tables$step3$table_insDb$symbol), anno)
en5 <- step1(en5)
