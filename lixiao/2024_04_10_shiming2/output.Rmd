...
```{r include = F, eval = F}
deparse_mail()
info <- items(belong = odate(4), eval = ic(0, 1, 0, 0, 0), lock = F)
show.ic(info)
...
autor(wrap(hb@params$p.pharm2hirudin, 7, 4))
```
...
```{r}
hb <- map(hb, gm@params$lst.genes, edge_width = 1, compounds = "hirudin", name = "hirudin")
...
```{r}
vn <- job_vina(c(hirudin = 72941487), hb@params$p.pharm2hirudin$.data$Target.name)
vn <- step1(vn, pdbs = c(SELP = "1g1q"))
vn <- step2(vn)
vn <- step3(vn)
```
