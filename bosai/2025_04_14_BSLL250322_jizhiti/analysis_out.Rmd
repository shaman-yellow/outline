...
order_packaging.complex(
  "./analysis_out.docx", idname = idname
  # , extras = c("./Reply_FL.docx", "./Reply_FL2.docx")
)
```
...
geo.sc_luad$guess
geo.sc_luad <- step2(geo.sc_luad, rna = FALSE, hasFile = TRUE)
geo.sc_luad@params$dir_files
...
```{r}
files_scRNA <- list.files(geo.sc_luad@params$dir, "TD", full.names = TRUE)
...
infe.epithelial <- step1(infe.epithelial, 8)
clear(infe.epithelial)
...
```{r}
# clear(srn.luad)
srn.luad <- readRDS("./srn.luad.6.rds")
cc.luad <- asjob_cellchat(srn.luad, "infercnv_cell", sample = .05)
clear_others("cc.luad")

# set_remoteRun.bosai(32)
#' @meth {get_meth(cc.luad)}
# cc.luad <- set_remote(cc.luad)
cc.luad <- step1(cc.luad, 4, testLocal = TRUE)
