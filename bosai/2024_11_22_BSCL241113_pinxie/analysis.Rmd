...
lm.aa_flux@tables$step2$tops$`Erythroblast_AA - Erythroblast_Normal`$name
...
```{r}
gds.aa_bulk <- job_gds(c("Aplastic anemia", "Healthy"))
vis(gds.aa_bulk)
gds.aa_bulk <- step1(gds.aa_bulk)
active(gds.aa_bulk, "GSE165870")
```
...
dplyr::filter(lm.aa_bulk@tables$step2$tops$`non_SAA - Ctrl`, GeneSymbol == "TUBB2A")
