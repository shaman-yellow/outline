...
lm.aa_flux@plots$step2$p.volcano$`B_cell_AA - B_cell_Normal`
...
feature(sr5n.aa, "cfrom")

sr5n.aa@params$res_correlation$Mature_endothelial_cell$hp
sr5n.aa@params$res_correlation$Platelet$hp
sr5n.aa@params$res_correlation$Neural_stem_cell$hp
sr5n.aa@params$res_correlation$B_cell$hp
```

## Monocle3 拟时分析 (AA_COR)

`r snap(mn.aa_cor, 0:4)`

```{r}
mn.aa_cor <- do_monocle(
  sr5n.aa, bind(names(feature(sr5n.aa, "cfrom")), co = "|"),
  group.by = "Cell_Sample"
)
#' @meth {get_meth(mn.aa_cor)}
mn.aa_cor <- step1(mn.aa_cor, "scsa_cell", pre = TRUE)
mn.aa_cor@plots$step1$p.prin
mn.aa_cor@plots$step1$p.traj
```
