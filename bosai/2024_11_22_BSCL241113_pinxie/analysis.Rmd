...
`r ref("AA-ChatGPT-Cell-type-annotation")` 
`r ref("AA-CellMarker-Validation")`
`r ref("AA-ChatGPT4-Cell-Proportions-in-each-sample")` 
...
cm.bone_marrow <- job_markers("bone.*marrow|blood")
cm.bone_marrow <- step1(
  cm.bone_marrow, c(
    "Erythroblast", "Macrophage", "Mesenchymal", "Monocyte", 
    "Neutrophil", "Plasma", "Platelet"
  )
)
cm.bone_marrow
sr5n.aa <- map(sr5n.aa, cm.bone_marrow, "CSF3R", 12)
...
```{r eval = T, echo = F, results = "asis"}
#| AA-ChatGPT-Cell-type-annotation
autor(sr5n.aa@plots$step6$p.map_gpt)
```
...
```{r eval = T, echo = F, results = "asis"}
#| AA-ChatGPT4-Cell-Proportions-in-each-sample
autor(sr5n.aa@plots$step6$p.props_gpt)
```


```{r eval = T, echo = F, results = "asis"}
#| AA-The-Cell-Sample
autor(wrap(p.cell_sample, 10, 5))
```
...
`r snap(sr5n.aa, "m")`
`r ref("AA-Filtered-TWAS-associated-genes-of-Cell-Cluster-DEGs")`

```{r}
sr5n.aa <- map(sr5n.aa, fus.aa)
sr5n.aa@params$fusion_degs
split(sr5n.aa@params$fusion_degs, ~ contrast)

tbl_genesEry <- dplyr::filter(sr5n.aa@params$fusion_degs, grpl(contrast, "Erythroblast"))
tbl_genesEry$gene
...
```{r eval = T, echo = F, results = "asis"}
#| AA-Filtered-TWAS-associated-genes-of-Cell-Cluster-DEGs
autor(sr5n.aa@params$fusion_degs)
```
...
`r ref("AA-FLUX-Erythroblast-AA-vs-Erythroblast-Normal")` 
...
lm.aa_flux@tables$step2$tops$`Erythroblast_AA - Erythroblast_Normal`
tbl_lm.aa_flux_Eryth <- lm.aa_flux@tables$step2$tops$`Erythroblast_AA - Erythroblast_Normal`
less(tbl_lm.aa_flux_Eryth$name)
lm.aa_flux@plots$step2$p.volcano$`Erythroblast_AA - Erythroblast_Normal`

character_lm.aa_flux_name <- lm.aa_flux@tables$step2$tops$`Erythroblast_AA - Erythroblast_Normal`$name
character_lm.aa_flux_name
```
...
```{r eval = T, echo = F, results = "asis"}
#| AA-FLUX-Erythroblast-AA-vs-Erythroblast-Normal
autor(lm.aa_flux@plots$step2$p.volcano$`Erythroblast_AA - Erythroblast_Normal`)
```
