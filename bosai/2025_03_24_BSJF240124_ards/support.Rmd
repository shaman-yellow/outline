...
```{r}
tf.bmp9 <- map(tf.bmp9, feature(venn.bmp9))
tf.bmp9@params$map_data_TF
tf.bmp9@params$p.logos_TF$STAT1
plot_region(tf.bmp9@params$map_data_TF, "ch10")
...
ranges <- lapply(c("promoters", "transcripts", "exons", "terminators"),
  function(type) {
    ...
    range$type <- type
    Gviz::GeneRegionTrack(range, name = Hmisc::capitalize(type))
  })
ranges

gtrack <- Gviz::GenomeAxisTrack()
atrack <- Gviz::AnnotationTrack()
plotTracks(c(list(gtrack), ranges), sizes = c(1, rep(1, 4)))
Gviz::UcscTrack()
