...
order_packaging(
  "./opt_out.docx", idname = idname, external_file = NULL, extras = NULL
)
```
...
mn.os_cancer <- do_monocle(
  srn.os_cancer, c("OS"), group.by = "scsa_cell", sct = TRUE
)

mn.os_cancer <- step1(mn.os_cancer, "scsa_cell", pre = TRUE)
...
`r ref("TCGA-OS-lasso-COX-coeffients-lambda-min")`
`r ref("TCGA-OS-lasso-COX-coeffients-lambda-1se")`
...
```{r eval = T, echo = F, results = "asis"}
#| TCGA-OS-lasso-COX-coeffients-lambda-min
autor(cox.tcga_os@plots$step4$p.lassoCOX_coeffients$lambda.min)
```


```{r eval = T, echo = F, results = "asis"}
#| TCGA-OS-lasso-COX-coeffients-lambda-1se
autor(cox.tcga_os@plots$step4$p.lassoCOX_coeffients$lambda.1se)
```
...
autor(surv.tcga_os@plots$step1$p.surv)
```


```{r eval = T, echo = F, results = "asis"}
#| TCGA-OS-ROC-plots
autor(surv.tcga_os@plots$step1$p.roc)
```
