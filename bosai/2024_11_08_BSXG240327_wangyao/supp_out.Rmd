...
```{r include = F, eval = F}
info <- items(start = td("20240909"), end = td("20241115"), finish = td(""),
...
order_publish("supp.Rmd", "supp_out.Rmd")
file.rename("./supp_out.pdf", paste0(info$id, "补充分析.pdf"))
...
见 Fig. \@ref(fig:MRNA-Mfuzz-clusters)。`r snap(mfuzz.mrna, c(1, 3, 4), c(6, 8))`。
...
```{r eval = T, echo = F, results = "asis"}
#| LUSC-metadata-stats
summary_tibble(lm.lusc$t.common)
```
...
```{r eval = T, echo = F, results = "asis"}
summary_tibble(metadata.geolusc)
```
