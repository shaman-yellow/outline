...
dplyr::filter(lm.ch_GSE44076@tables$step2$tops$`Tumor_sample - Mucosa_sample`,
  grpl(hgnc_symbol, "PIEZO1|FAM38A", TRUE)
)
lm.ch_GSE44076 <- map(lm.ch_GSE44076, "FAM38A", ref.use = "hgnc_symbol")
lm.ch_GSE44076@plots$step2$p.BoxPlotOfDEGs
...
dplyr::filter(lm.cn_GSE44076@tables$step2$tops$`Tumor_sample - Normal_paired_sample`,
  grpl(hgnc_symbol, "PIEZO1|FAM38A", TRUE)
)
lm.cn_GSE44076@tables$step2$tops$`Tumor_sample - Normal_paired_sample`
lm.cn_GSE44076 <- map(lm.cn_GSE44076, "FAM38A", ref.use = "hgnc_symbol")
lm.cn_GSE44076@plots$step2$p.BoxPlotOfDEGs
```
...
dplyr::filter(lm.cn_GSE95132@tables$step2$tops$`Colon_cancer_stage_II_IV - Healthy`,
  grpl(hgnc_symbol, "PIEZO1|FAM38A", TRUE)
)
lm.cn_GSE95132@tables$step2$tops$`Colon_cancer_stage_II_IV - Healthy`
lm.cn_GSE95132 <- map(lm.cn_GSE95132, "PIEZO1")
lm.cn_GSE95132@plots$step2$p.BoxPlotOfDEGs
```
...
dplyr::filter(lm.cn_GSE137327@tables$step2$tops$`Primary_colon_cancer - Adjacent_non_cancerous_colon_tissue`,
  grpl(hgnc_symbol, "PIEZO1|FAM38A", TRUE)
)
lm.cn_GSE137327@tables$step2$tops$`Primary_colon_cancer - Adjacent_non_cancerous_colon_tissue`
lm.cn_GSE137327 <- map(lm.cn_GSE137327, "PIEZO1")
lm.cn_GSE137327@plots$step2$p.BoxPlotOfDEGs
```
...
dplyr::filter(lm.cn_GSE76855@tables$step2$tops$`Colon_Cancer - Normal_tissue`,
  grpl(GENE_SYMBOL, "PIEZO1|FAM38A", TRUE)
)
lm.cn_GSE76855@tables$step2$tops$`Colon_Cancer - Normal_tissue`
lm.cn_GSE76855 <- map(lm.cn_GSE76855, "PIEZO1")
...
dplyr::filter(lm.cn_GSE38939@tables$step2$tops$`tumor - adjacent_non_tumor`,
  grpl(Symbol, "PIEZO1|FAM38A", TRUE)
)
lm.cn_GSE38939@tables$step2$tops$`tumor - adjacent_non_tumor`
lm.cn_GSE38939 <- map(lm.cn_GSE38939, "FAM38A", ref.use = "Symbol")
...
```{r}
fun_extract_box <- function(names) {
  sapply(names, simplify = FALSE,
    function(x) {
      x <- get(x, envir = .GlobalEnv)
      x@plots$step2$p.BoxPlotOfDEGs
    })
}

gene_piezo1_ch <- ls(pattern = "lm.xena|lm.ch_")
# [1] "lm.ch_GSE44076" "lm.xena_coad"  
gene_piezo1_ch <- fun_extract_box(gene_piezo1_ch)
gene_piezo1_ch <- frame_wrap(gene_piezo1_ch)
gene_piezo1_ch <- wrap(gene_piezo1_ch, 6, 4)
gene_piezo1_ch <- set_lab_legend(gene_piezo1_ch, "PIEZO1 Cancer vs Healthy")
gene_piezo1_ch

gene_piezo1_cn <- ls(pattern = "lm.cn_")
gene_piezo1_cn <- gene_piezo1_cn[!grpl(gene_piezo1_cn, "GSE137327|GSE38939")]
gene_piezo1_cn <- fun_extract_box(gene_piezo1_cn)
gene_piezo1_cn <- frame_wrap(gene_piezo1_cn)
...
dplyr::filter(
  lm.surv_GSE29621@object$genes, grpl(hgnc_symbol, "PIEZO1")
)
