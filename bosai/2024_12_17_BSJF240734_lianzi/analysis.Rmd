...
order_packaging(
  "./analysis_out.docx", idname = idname, external_file = NULL, extras = "./Reply_闫姐.docx"
)
```
...
`r ref("COMMON-hsa04066-visualization")` 
...
`r snap(lm.ckd, 0:2)`
针对 `r snap(feature(en.common), TRUE)` 差异分析。
`r snap(lm.ckd, 3)`
`r snap(p.less)`
...
lm.ckd <- filter(lm.ckd, GENE_SYMBOL %in% feature(en.common))
lm.ckd <- step1(lm.ckd)
lm.ckd <- step2(
...
p.less <- wrap(map(lm.ckd, c("VEGFA", "TLR4")), 4, 4)
p.less
```
...
```{r eval = T, echo = F, results = "asis"}
#| CKD-wilcox-test-for-some-genes
autor(p.less)
```
...
以 GEO 数据验证后发现，`r feature(p.less)` 为差异表达基因。
