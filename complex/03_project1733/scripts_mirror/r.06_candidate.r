...
output <- file.path(ORIGINAL_DIR, "06_candidate")
...
hdw.GSE150825_sciPos_FibroClean <- readRDS("./rds_jobSave/hdw.GSE150825_sciPos_FibroClean.5.rds")
# gn.serotonin <- readRDS("./rds_jobSave/gn.serotonin.1.rds")
au.GSE150825 <- readRDS("./rds_jobSave/lite/au.GSE150825.1.rds")

ven.candidates <- job_venn(
  `DEGs (NPC vs control)` = feature(lm.GSE53819),
  `DEGs (IBL_High vs IBL_Low)` = feature(des.GSE189642),
  `Serotonin Genes` = feature(au.GSE150825, "gene"),
  `Module Genes` = as_feature(hdw.GSE150825_sciPos_FibroClean$modules$gene_name, "All Module Genes"),
  analysis = "候选基因"
)
...
if (FALSE) {
    # ven.candidates <- job_venn(`DEGs (NPC vs control)` = feature(lm.GSE53819), 
    #     `DEGs (IBL_High vs IBL_Low)` = feature(des.GSE189642), `Serotonin Genes` = feature(au.GSE150825, 
    #         "gene"), `Module Genes` = as_feature(hdw.GSE150825_sciPos_FibroClean$modules$gene_name, 
    #         "All Module Genes"), analysis = "候选基因")
