...
output <- file.path(ORIGINAL_DIR, "05_DEGs")
...
geo.GSE53819 <- job_geo("GSE53819")
geo.GSE53819 <- step1(geo.GSE53819)

metadata.GSE53819 <- expect(geo.GSE53819, geo_cols())
metadata.GSE53819 <- dplyr::mutate(
  metadata.GSE53819, group = ifelse(
    grpl(group, "Non_"), "control", "NPC"
  )
)

lm.GSE53819 <- asjob_limma(geo.GSE53819, metadata.GSE53819)
lm.GSE53819 <- step1(lm.GSE53819)
lm.GSE53819 <- step2(
  lm.GSE53819, NPC - control,
  use = "P", cut.fc = .5
)
lm.GSE53819 <- step3(lm.GSE53819)
clear(lm.GSE53819)
...
if (FALSE) {
    ...
    # lm.GSE53819 <- step2(lm.GSE53819, NPC - control, use = "P", cut.fc = 0.5)
    ...
    setMethod(f = "clear", signature = c(x = "job_limma"), definition = function (x, 
        ...) 
    {
        .local <- function (x, save = TRUE, suffix = NULL, name = substitute(x, 
            parent.frame(1))) 
        {
            eval(name)
