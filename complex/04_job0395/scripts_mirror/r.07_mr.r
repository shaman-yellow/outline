...
output <- file.path(ORIGINAL_DIR, "07_mr")
...
ven.key <- readRDS("./rds_jobSave/ven.key.0.rds")
fea.key <- feature(ven.key)
...
fea.markers <- as_feature(
  resolve_feature(feature(mr.key, "exposure")), "脓毒症相关核心生物标志物"
)
clear_feature(fea.markers)

mr.key$data_outcome$outcome
...
output_with_counting_number({
  mr.key@plots$step5$p.forest
  mr.key@plots$step5$p.scatter
  mr.key@plots$step5$p.funnel
  mr.key@plots$step5$p.leaveoneout
  mr.key@tables$step5$t.main
  mr.key@tables$step5$t.heterogeneity
  mr.key@tables$step5$t.pleiotropy
  mr.key@tables$step5$t.steiger
  notshow(mr.key@params$data_exposure_raw)
  notshow(mr.key@params$data_exposure)
  notshow(mr.key@params$data_outcome)
  notshow(mr.key@params$fTest$data_f)
  notshow(mr.key@params$table_with_all_columns)
  feature(mr.key, "exposure")
})
...
if (FALSE) {
    ...
    setMethod(f = "asjob_mr", signature = c(x = "feature"), definition = function (x, 
        ...) 
    {
        .local <- function (x, mode = "eqtlgen", strict = TRUE, ...) 
        {
            mode <- match.arg(mode)
    ...
    # mr.key <- step1(mr.key, top_n = 1L, check_new_snps = TRUE, templates = c("smoking", 
    #     "drinking", "bmi", "education", "diabetes", "blood_pressure", 
    #     "lipid", "crp", "immune", "infection"))
    ...
    # mr.key <- step2(mr.key, "general", c("ieu-b-5088"), check_new_snps = TRUE)
    setMethod(f = "step2", signature = c(x = "job_mr"), definition = function (x, 
        ...) 
    {
        .local <- function (x, mode = c("mbg", "general"), ids = NULL, 
            ...) 
        {
            ...
            if (mode == "general") {
                ...
                data_outcome <- .get_outcome_opengwas_general(x$data_exposure$SNP, 
                    ids, ...)
    ...
    setMethod(f = "step3", signature = c(x = "job_mr"), definition = function (x, 
        ...) 
    {
        .local <- function (x, filter_outcome = TRUE, cut.p = 1e-05, 
            ...) 
        {
        ...
        ...) 
    {
        ...
        if (x$split_exposure == "SYMBOL") {
            x$.feature_exposure <- as_feature(split(t.main$exposure, 
                t.main$outcome), "MR 显著因果关联")
        }
        args$wrap <- TRUE
    ...
    # fea.markers <- as_feature(resolve_feature(feature(mr.key, "exposure")), 
    #     "脓毒症相关核心生物标志物")
