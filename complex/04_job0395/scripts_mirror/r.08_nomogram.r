...
output_with_counting_number({
  rms.GSE134347@plots$step1$p.nomo
  notshow(rms.GSE134347@params$res_lrm$data)
  rms.GSE134347@plots$step1$p.rocs
  rms.GSE134347@plots$step1$p.cal
  rms.GSE134347@plots$step2$p.dcas
})
...
if (FALSE) {
    # rms.GSE134347 <- asjob_rms(lm.GSE134347, fea.markers)
    setMethod(f = "asjob_rms", signature = c(x = "job_limma"), definition = function (x, 
        ...) 
    {
        .local <- function (x, ref, target = "group", format_name = TRUE, 
            levels = rev(.guess_compare_limma(x, 1L))) 
        {
            if (x@step < 1L) {
                stop("x@step < 1L.")
            }
