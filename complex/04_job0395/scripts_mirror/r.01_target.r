...
output <- file.path(ORIGINAL_DIR, "01_target")
...
if (FALSE) {
  ...
  data.table::fwrite(data_cpd, "./compounds.csv")
}

fig.search <- .file_fig("./sepsis_search.pdf")
fig.search <- set_lab_legend(
  as_data_binary(fig.search),
  glue::glue("Literature search"),
  glue::glue("文献检索|||通过PubMed、CNKI、WebofScience、万方数据库检索“脓毒症+针灸+内源性活性物质”相关文献")
)
j.search <- .job()
j.search$fig.search <- fig.search
clear(j.search)
...
# file.copy("./literature.pdf", "./01_target/00_literature.pdf", TRUE)
...
#| OVERTURE
output_with_counting_number({
  notshow(sup.cpd@params$data_target)
  feature(sup.cpd, "target")
})
...
if (FALSE) {
    # fig.search <- set_lab_legend(as_data_binary(fig.search), glue::glue("Literature search"), 
    #     glue::glue("文献检索|||通过PubMed、CNKI、WebofScience、万方数据库检索“脓毒症+针灸+内源性活性物质”相关文献"))
