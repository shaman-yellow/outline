...
output <- file.path(ORIGINAL_DIR, "16_monocle")
...
mn.GSE167363_T_Clean <- asjob_monocle2(
  srn.GSE167363_T_Clean
)

mn.GSE167363_T_Clean <- step1(mn.GSE167363_T_Clean)
mn.GSE167363_T_Clean <- step2(mn.GSE167363_T_Clean, "diff", top = 150)
mn.GSE167363_T_Clean <- step3(mn.GSE167363_T_Clean)

fea.markers <- load_feature()

mn.GSE167363_T_Clean <- step4(
  mn.GSE167363_T_Clean, fea.markers,
  relative_expr = FALSE
)

mn.GSE167363_T_Clean <- step5(
  mn.GSE167363_T_Clean, 2L, list(c(3, 5), c(4, 5)),
  features = fea.markers
)
clear(mn.GSE167363_T_Clean)
...
output_with_counting_number({
  z7(mn.GSE167363_T_Clean@plots$step3$p.traj, .7, .7)
  mn.GSE167363_T_Clean@plots$step4$p.geneInPseudo$scsa_cell
  z7(mn.GSE167363_T_Clean@plots$step5$p.hp, .7, .3)
  notshow(mn.GSE167363_T_Clean@params$diff_genes)
})
...
if (FALSE) {
    ...
    # mn.GSE167363_T_Clean <- asjob_monocle2(srn.GSE167363_T_Clean)
        ...
        definition = function (x, ...) 
        {
                ...
                compare.by = "group", group.by = x$group.by, nfeatures = 1000) 
            {
                ...
                if (!is.null(compare)) {
                    ...
                    if (SeuratObject::DefaultAssay(object(x)) == 
                      "SCT" && !is.null(x$seurat_subset) && x$seurat_subset) {
    ...
    # mn.GSE167363_T_Clean <- step4(mn.GSE167363_T_Clean, fea.markers, 
    #     relative_expr = FALSE)
    ...
    # mn.GSE167363_T_Clean <- step5(mn.GSE167363_T_Clean, 2L, list(c(3, 
    #     5), c(4, 5)), features = fea.markers)
    setMethod(f = "step5", signature = c(x = "job_monocle2"), definition = function (x, 
        ...) 
    {
        .local <- function (x, point = NULL, list_branches = NULL, 
            maxShow = 50, workers = 1, rerun = FALSE, features = NULL) 
        {
            ...
            if (!is.null(features)) {
                ...
                fun_cache <- function(...) {
                    ...
                    callr::r(run_diff, list(cds = object(x)[genes, 
                      ], cores = workers), libpath = .libPaths(), 
                      show = FALSE)
                }
                args <- x$.args[names(x$.args) %in% paste0("step", 
                    1:4)]
                diff_test_pseudotime <- expect_local_data("tmp", 
                    "monocle_diff", fun_cache, list(args), rerun = rerun)
                ...
                x$diff_test_pseudotime <- diff_test_pseudotime <- dplyr::arrange(diff_test_pseudotime, 
                    qval)
                data <- dplyr::filter(diff_test_pseudotime, qval < 
                    0.05)
            ...
            fun_heatmap <- function(cds, point, n_cluster, list_branches) {
                ...
                if (!is.null(point)) {
                    if (is.null(list_branches)) {
                      monocle::plot_genes_branched_heatmap(cds, branch_point = point, 
                        num_clusters = n_cluster, show_rownames = TRUE, 
                        return_heatmap = TRUE)
                    }
                    else {
                      lapply(list_branches, function(branch_states) {
                        if (length(branch_states) != 2) {
                          stop("length(branch_states) != 2.")
                        }
                        branch_labels <- paste0("Cell state ", branch_states)
                        message(glue::glue("Compare cell state: {paste(branch_states, collapse = ' vs ')}"))
                        monocle::plot_genes_branched_heatmap(cds, 
                          branch_point = point, branch_states = branch_states, 
                          branch_labels = branch_labels, num_clusters = n_cluster, 
                          show_rownames = TRUE, return_heatmap = TRUE)
                      })
                    }
                }
                else {
                    monocle::plot_pseudotime_heatmap(cds, num_clusters = n_cluster, 
                      show_rownames = TRUE, return_heatmap = TRUE)
                }
            }
            p.hp <- callr::r(fun_heatmap, list(cds = object(x)[genes, 
                ], point = point, n_cluster = n_cluster, list_branches), 
                libpath = .libPaths(), show = TRUE)
            if (!is.null(point)) {
                x$hp_raw <- p.hp
                if (is.null(list_branches)) {
                    grob <- p.hp$ph_res$gtable
                    p.hp <- wrap_scale_heatmap(grob, 12, length(genes), 
                      pre_height = 3L, raw = FALSE)
                }
                else {
                    layout <- wrap_layout(NULL, length(list_branches), 
                      f.w = 1.5)
                    p.hp <- patchwork::wrap_plots(lapply(p.hp, function(x) x$ph_res$gtable), 
                      ncol = layout$ncol)
                    p.hp <- add(layout, p.hp)
                }
                p.hp <- set_lab_legend(p.hp, glue::glue("{x@sig} pseudotime genes in heatmap with branch"), 
                ...
                p.hp <- set_lab_legend(wrap(p.hp$gtable, 5, 8), glue::glue("{x@sig} pseudotime genes in heatmap"), 
