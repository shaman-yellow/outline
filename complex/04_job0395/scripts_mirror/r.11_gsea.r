...
output <- file.path(ORIGINAL_DIR, "11_gsea")
...
corgsea.marker <- asjob_corgsea(lm.GSE134347, fea.markers)
...
output_with_counting_number({
  wrap(corgsea.marker@plots$step2$p.codes$ITK, 9, 7)
  notshow(corgsea.marker@tables$step1$table_gsea$ITK)
  wrap(corgsea.marker@plots$step2$p.codes$RPS6KA5, 9, 7)
  notshow(corgsea.marker@tables$step1$table_gsea$RPS6KA5)
})
...
if (FALSE) {
    # corgsea.marker <- asjob_corgsea(lm.GSE134347, fea.markers)
    setMethod(f = "asjob_corgsea", signature = c(x = "job_limma"), 
