...
sepsis_celltype_db <- list(
  ...
  Monocyte = list(
    ...
    pmid = c("34558746", "38504349")
  ),

  Dendritic_Cell = list(
    markers = c(
      "FCER1A", "CST3", "HLA-DRA",
      "HLA-DPA1", "CD74", "CLEC10A"
    ),
    pmid = c("34558746")
  ),
...
show.markers <- as_markers(sepsis_celltype_db)

ref.markers <- show.markers

requireNamespace("Seurat")
srn.GSE167363 <- step6(
  srn.GSE167363, "", ref.markers, show.markers, 
  forceCluster = list("Neutrophil" = c(2L, 19L, 3L, 12L, 17, 27L))
)

fea.markers <- load_feature()

srn.GSE167363 <- focus(
  srn.GSE167363, fea.markers, name = "by_cell"
)
srn.GSE167363 <- focus(
  srn.GSE167363, fea.markers, "group", name = "by_group", facetTest = "scsa_cell"
)
clear(srn.GSE167363)
...
#| OVERTURE
output_with_counting_number({
  srn.GSE167363@params$focus_by_cell$p.dim
  srn.GSE167363@params$focus_by_cell$p.dot
  srn.GSE167363@params$focus_by_group$plist.vlnFacet$ITK
  srn.GSE167363@params$focus_by_group$plist.vlnFacet$RPS6KA5
})
...
if (FALSE) {
    ...
    # srn.GSE167363 <- job_seurat5n(files, strx(basename(files), "GSM[0-9]+"))
    job_seurat5n <- function (dirs, names = NULL, mode = c("sc", "st"), st.filename = "filtered_feature_bc_matrix.h5", 
        ...) 
    {
        mode <- match.arg(mode)
        n <- 0L
        object <- pbapply::pblapply(dirs, function(dir) {
            n <<- n + 1L
            project <- names[n]
            if (mode == "sc") {
                suppressMessages(job_seurat(dir, project = project, 
                    ...))@object
            }
            else {
                suppressMessages(job_seuratSp(dir, filename = st.filename, 
                    ...))@object
            }
        })
        x <- .job_seurat5n(object = object)
        object(x) <- e(SeuratObject:::merge.Seurat(object(x)[[1]], 
            object(x)[-1]))
    ...
    # srn.GSE167363 <- step6(srn.GSE167363, "", ref.markers, show.markers, 
    #     forceCluster = list(Neutrophil = c(2L, 19L, 3L, 12L, 17, 
    #         27L)))
    ...
    # srn.GSE167363 <- focus(srn.GSE167363, fea.markers, name = "by_cell")
    setMethod(f = "focus", signature = c(x = "job_seurat"), definition = function (x, 
        ...) 
    {
        .local <- function (x, features, group.by = x@params$group.by, 
            name = "genes", recode = NULL, vln = TRUE, dim = TRUE, 
            dot = TRUE, cols = c("lightgrey", "blue"), facetTest = NULL, 
            levels = .guess_levels_from_job_seurat(x), return_type = c("job", 
                "list"), sp = FALSE, ...) 
        {
            ...
            renameFeature <- FALSE
            if (!is.null(recode)) {
                refea <- dplyr::recode(features, !!!setNames(names(recode), 
                    unname(recode)))
                renameFeature <- TRUE
            ...
            ps <- list()
            if (vln) {
                p.vln <- e(Seurat::VlnPlot(object(x), features = refea, 
                    group.by = group.by, pt.size = 0, alpha = 0.3, 
                    cols = color_set(), combine = FALSE))
                if (renameFeature) {
                    p.vln <- lapply(p.vln, .set_ggplot_content, fun = fun_rename_title, 
                      slot = "labels")
                }
                if (!is.null(facetTest)) {
                    ps$plist.vlnFacet <- .facet_seurat_vlnPlot(p.vln, 
                      facetTest, x, recode = recode)
                    snaps <- vapply(names(ps$plist.vlnFacet), FUN.VALUE = character(1), 
                      function(name) {
                        .stat_compare_by_pvalue(ps$plist.vlnFacet[[name]], 
                          levels, name, recode = recode)
                      })
                    snaps <- bind(snaps, co = "\n\n\n")
                    x <- snapAdd(x, "聚焦于 {bind(features)} 在各细胞中的表达趋势，使用 Wilcoxon 检验对基因表达量差异分析 (P &lt; 0.05)。{snaps}。", 
                      add = FALSE, step = name)
                }
                p.vln <- lapply(p.vln, function(x) x + theme(legend.position = "none"))
                p.vln <- wrap_layout(patchwork::wrap_plots(p.vln, 
                    ncol = ncol), layout)
                p.vln <- set_lab_legend(p.vln, glue::glue("{x@sig} violing plot of {name}"), 
                    glue::glue("基因 {less(features)} 表达水平的小提琴图。|||横坐标为各类细胞，纵坐标为基因表达量，小提琴图展示了该类型细胞的基因表达量的整体分布。"))
                ps$p.vln <- p.vln
            }
            if (dim) {
                p.dim <- e(Seurat::FeaturePlot(object(x), features = refea, 
                    combine = FALSE, cols = cols))
                if (renameFeature) {
                    p.dim <- lapply(p.dim, .set_ggplot_content, fun = fun_rename_title, 
                      slot = "labels")
                }
                p.dim <- wrap_layout(patchwork::wrap_plots(p.dim, 
                    ncol = ncol), layout)
                p.dim <- set_lab_legend(p.dim, glue::glue("{x@sig} dimension plot of {name}"), 
                    glue::glue("基因 {bind(features)} 表达水平的 Dimension reduction plot|||基因的表达量在降维后的图中的直观呈现，表达量越高，颜色越深。"))
                ps$p.dim <- p.dim
            }
            if (dot) {
                p.dot <- e(Seurat::DotPlot(object(x), features = refea, 
                    group.by = group.by)) + theme(axis.text.x = element_text(angle = 90, 
                    vjust = 0.5, hjust = 1))
                if (renameFeature) {
                    fun_mutate <- function(data) {
                      dplyr::mutate(data, features.plot = dplyr::recode(features.plot, 
                        !!!recode))
                    }
                    p.dot <- .set_ggplot_content(p.dot, fun_mutate)
                }
                p.dot <- set_lab_legend(wrap_scale_heatmap(p.dot, 
                    refea, object(x)@meta.data[[group.by]]), glue::glue("{x@sig} dot plot of {name}"), 
    ...
    # srn.GSE167363 <- focus(srn.GSE167363, fea.markers, "group", name = "by_group", 
    #     facetTest = "scsa_cell")
    setMethod(f = "focus", signature = c(x = "job_seurat"), definition = function (x, 
        ...) 
    {
