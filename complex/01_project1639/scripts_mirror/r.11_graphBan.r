# ==========================================================================
# FIELD: setup
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

rm(list = ls()); gc()
ORIGINAL_DIR <- "/data/nas1/huanglichuang_OD/project/01_project1639"
output <- file.path(ORIGINAL_DIR, "11_graphBan")
if (!dir.exists(output)) {
  dir.create(output, recursive = TRUE)
}
setwd(ORIGINAL_DIR)

.libPaths(c('/data/nas2/software/miniconda3/envs/public_R/lib/R/library/', '/data/nas1/huanglichuang_OD/conda/envs/extra_pkgs/lib/R/library/'))

myPkg <- "./utils.tool"
if (!dir.exists(myPkg)) {
  stop('Can not found package: ', myPkg)
}
devtools::load_all(myPkg)
setup.huibang()

# ==========================================================================
# FIELD: analysis
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

fea.markers <- as_feature(c("H1-4", "FXYD7"), ref = "生物标识物基因")

gb.markers <- asjob_gBan(fea.markers)
gb.markers <- step1(gb.markers)
clear(gb.markers)

gb.markers <- step2(gb.markers)
gb.markers <- step3(gb.markers)
clear(gb.markers)

# load results file
gb.markers@step <- 3L
gb.markers <- step4(
  gb.markers, "graphBan_res_", cutoff = .95, method_keep = "all"
)
clear(gb.markers)

# gb.markers <- readRDS("./rds_jobSave/gb.markers.5.rds")
gb.markers <- step5(gb.markers, "./GraphBAN_MARKERS/res_admet.csv")
gb.markers <- step6(gb.markers, "./GraphBAN_MARKERS/res_swissadme.csv")
gb.markers@step <- 6L
gb.markers <- step7(gb.markers)
clear(gb.markers)

vn.drugs <- readRDS("./rds_jobSave/vn.drugs.7.rds")
clear(vn.drugs)


# ==========================================================================
# FIELD: output
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

setup_counting_in_directory(output)

#| OVERTURE
output_with_counting_number({
  notshow(gb.markers@params$info_compounds)
  notshow(gb.markers@params$split_by_genes)
  gb.markers@plots$step4$p.common
  gb.markers@tables$step7$t.final_candidates_mutate
  notshow(gb.markers@params$admet)
  notshow(gb.markers@params$admet_filter)
  notshow(gb.markers@params$swissAdme)
  notshow(gb.markers@tables$step7$t.final_candidates)
  z7(vn.drugs@plots$step5$p.res_vina, .7, .7)
  vn.drugs@tables$step5$t.sigData
  vn.drugs@plots$step6$figs$Top1_FXYD7_24893490
  vn.drugs@plots$step6$figs$`Top2_H1-4_98104494`
  vn.drugs@plots$step7$figs$Top1_FXYD7_24893490
  vn.drugs@plots$step7$figs$`Top2_H1-4_98104494`
})


